...
const encryptCredential = (obj) => {
  ...
  switch (vendor) {
    ...
    case 'amivoice':
      assert(api_key, 'invalid amivoice speech credential: api_key is required');
      const amivoiceData = JSON.stringify({api_key, model_id});
      return encrypt(amivoiceData);
...
router.get('/:sid/test', async(req, res) => {
  ...
  try {
    ...
    } else if (cred.vendor === 'amivoice') {
      const {api_key} = credential;
      if (cred.use_for_stt) {
        try {
          await testAmiVoiceStt(logger, {api_key});
          results.stt.status = 'ok';
          SpeechCredential.sttTestResult(sid, true);
        } catch (err) {
          results.stt = {status: 'fail', reason: err.message};
