...
const {decryptCredential, testWhisper, testDeepgramTTS,
  ...
  testHoundifyStt,
  testGladiaStt,
  testElevenlabsStt} = require('../../utils/speech-utils');
...
const encryptCredential = (obj) => {
  ...
  switch (vendor) {
    ...
    case 'elevenlabs':
      ...
      assert(stt_model_id, 'invalid elevenLabs speech credential: stt_model_id is required');
...
router.get('/:sid/test', async(req, res) => {
  ...
  try {
    ...
    if (cred.vendor === 'google') {
      ...
      if (cred.use_for_stt && api_key) {
        try {
          await testElevenlabsStt(logger, credential);
          results.stt.status = 'ok';
          SpeechCredential.sttTestResult(sid, true);
        } catch (err) {
          results.stt = {status: 'fail', reason: err.message};
