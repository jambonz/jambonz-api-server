...
const encryptCredential = (obj) => {
  ...
  switch (vendor) {
    ...
    case 'ibm':
      assert(tts_api_key, 'invalid ibm speech credential: tts_api_key is required');
      assert(tts_region, 'invalid ibm speech credential: tts_region is required');
      const ibmData = JSON.stringify({tts_api_key, tts_region});
...
router.get('/:sid/test', async(req, res) => {
  ...
  try {
    ...
    if (cred.vendor === 'google') {
      ...
      if (cred.use_for_stt) {
        try {
          //await testIbmStt(logger, {client_id, secret});
          results.stt.status = 'ok';
          SpeechCredential.sttTestResult(sid, true);
        } catch (err) {
          results.stt = {status: 'fail', reason: err.message};
