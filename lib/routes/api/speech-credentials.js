...
const {decryptCredential, testWhisper, testPlayHT} = require('../../utils/speech-utils');
...
const encryptCredential = (obj) => {
  ...
  switch (vendor) {
    ...
    case 'playht':
      ...
      const playhtData = JSON.stringify({api_key, user_id});
...
router.get('/:sid/test', async(req, res) => {
  ...
  try {
    ...
    if (cred.vendor === 'google') {
      ...
      if (cred.use_for_tts) {
        try {
          await testPlayHT(logger, credential);
          results.tts.status = 'ok';
          SpeechCredential.ttsTestResult(sid, true);
        } catch (err) {
          results.tts = {status: 'fail', reason: err.message};
...
`;
    ...
    const resp = await getJSON('https://api.play.ht/api/v2/voices');
    if (resp) {
      const buildVoice = (d) => {
        let name = `${d.name} -${concat(d.accent)}${concat(d.age)}
${concat(d.gender)}${concat(d.loudness)}${concat(d.style)}${concat(d.tempo)}${concat(d.texture)}` ;
        name = name.endsWith(',') ? name.slice(0, -1) : name;
        return {
          value: `${d.voice_engine}/${d.id}`,
          name
        };
      };
      const map = new Map();
      resp.forEach((v) => {
        if (map.has(v.language_code)) {
          map.get(v.language_code).voices.push(buildVoice(v));
        } else {
          map.set(v.language_code, {
            name: v.language,
