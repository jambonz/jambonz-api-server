...
router.delete('/:user_sid', async(req, res) => {
  ...
  const decodedJwt = jwt.verify(token, process.env.JWT_SECRET);
  const allUsers = await User.retrieveAll();
  const adminUsers = allUsers.filter((e) => !e.account_sid && !e.service_provider_sid);
  const user = await User.retrieve(user_sid);

  try {
    if (decodedJwt.scope === 'admin' && adminUsers.length === 1) {
      throw new Error('cannot delete this admin user - there are no other admin users');
    }
