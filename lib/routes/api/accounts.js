...
const updateCall = async(req, res) => {
  ...
  try {
    ...
    if (call) {
      ...
      if (!response.ok) {
        try {
          // If there is a body, past it back to the client
          const body = await response.json();
          logger.debug({body}, `updateCall: error response from ${url}`);
          return res.status(response.status).json(body);
        } catch (err) {
          logger.error({err}, `updateCall: error parsing json response from ${url}`);
