...
async function validateCreateCall(logger, sid, req) {
  ...
  const account = await lookupAccountBySid(sid);
  if (!account) throw new DbErrorBadRequest(`account not found for sid ${sid}`);
  if ((req.user.hasAccountAuth && req.user.account_sid !== sid) ||
    (req.user.hasServiceProviderAuth && account.service_provider_sid !== req.user.service_provider_sid)) {
