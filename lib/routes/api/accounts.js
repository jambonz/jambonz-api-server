...
const extractBucketCredential = (obj) => {
  const {bucket_credential, ...account} = obj;
  if (bucket_credential) {
    const o = JSON.parse(decrypt(bucket_credential));
    switch (o.vendor) {
      case 'aws_s3':
        account.bucket_credential = {
          ...o,
          secret_access_key: obscureKey(o.secret_access_key)
        };
        break;
    }
  }
...
router.get('/', async(req, res) => {
  ...
  try {
    ...
    const accounts = results.map((a) => extractBucketCredential(a));
...
router.get('/:sid', async(req, res) => {
  ...
  try {
    ...
    const account = extractBucketCredential(results[0]);
...
function encryptBucketCredential(obj) {
  ...
  switch (vendor) {
    case 'aws_s3':
      ...
      const awsData = JSON.stringify({vendor, name, access_key_id, secret_access_key});
    ...
    case 'none':
      ...
      break;
    default:
      throw DbErrorBadRequest(`unknow bucket vendor ${vendor}`);
  }
}
...
router.post('/:sid/BucketCredentialTest', async(req, res) => {
  ...
  try {
    const account_sid = parseAccountSid(req);
    await validateRequest(req, account_sid);
    let {vendor, name, access_key_id, secret_access_key} = req.body;
    const ret = {
      status: 'not tested'
    };
    ...
    switch (vendor) {
      case 'aws_s3':
        await testAwsS3(logger, {vendor, name, access_key_id, secret_access_key});
        ret.status = 'ok';
        break;
      default:
