...
router.get('/:sid', async(req, res) => {
  ...
  try {
    ...
    const {bucket_credential, ...account} = results[0];
    if (account.bucket_vendor && bucket_credential) {
      const o = JSON.parse(decrypt(bucket_credential));
      switch (account.bucket_vendor) {
        case 'aws_s3':
          account.bucket_access_key_id = o.bucket_access_key_id;
          account.bucket_secret_access_key = obscureKey(o.bucket_secret_access_key);
          account.bucket_name = o.bucket_name;
          break;
      }
    }
...
function encryptBucketCredential(obj) {
  ...
  switch (bucket_vendor) {
    case 'aws_s3':
      assert(bucket_access_key_id, 'invalid aws S3 bucket credential: access_key_id is required');
      assert(bucket_secret_access_key, 'invalid aws S3 bucket credential: secret_access_key is required');
      assert(bucket_name, 'invalid aws speech credential: aws_region is required');
      const awsData = JSON.stringify({bucket_name, bucket_access_key_id, bucket_secret_access_key});
      obj.bucket_credential = encrypt(awsData);
      delete obj.bucket_name;
      delete obj.bucket_access_key_id;
      delete obj.bucket_secret_access_key;
      break;
  }
}
...
router.get('/:sid/BucketCredentialTest', async(req, res) => {
  const logger = req.app.locals.logger;
  try {
    const account_sid = parseAccountSid(req);
    await validateRequest(req, account_sid);
    ...
    if (account.bucket_vendor && bucket_credential) {
      ...
      switch (account.bucket_vendor) {
        case 'aws_s3':
          if (testAwsS3(logger, o)) {
            ret.status = 'ok';
            Account.bucketTestResult(account_sid, true);
          } else {
            ret.status = 'fail';
            Account.bucketTestResult(account_sid, false);
          }
          break;
      }
      logger.info({o}, 'retrieving aws_s3 credential');
    } else {
      throw new DbErrorBadRequest('Account does not have any bucket credential to test.');
    }
