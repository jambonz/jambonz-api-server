...
async function validateCreateCall(logger, sid, req) {
  ...
  if (obj.application_sid) {
    ...
    let speech_creds = await SpeechCredential.retrieveAll(req.user.account_sid);
    if (!speech_creds || speech_creds.length === 0) {
      // try to fetch speech cred from service provider
      const {lookupAccountBySid} = req.app.locals;
      const account = await lookupAccountBySid(req.user.account_sid);
      speech_creds = await SpeechCredential.retrieveAllForSP(account.service_provider_sid);
    }
