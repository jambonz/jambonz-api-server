...
router.get('/:call_sid/logs', async(req, res) => {
  ...
  const account_sid = parseAccountSid(req.originalUrl);
  if (!aws_region) {
    return res.status(400).send({msg: 'logs is available only in AWS environment'});
  }
  if (!account_sid) {
  ...
  try {
    ...
    const startTime = new Date(attempted_at).getTime();
    const endTime = new Date(terminated_at).getTime();
    const client = new CloudWatchLogsClient({ region: aws_region });
    const params = {
      logGroupName,
      startTime,
      endTime,
      filterPattern: `{ $.callSid = "${call_sid}" }`
    };
