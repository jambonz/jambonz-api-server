...
router.get('/:call_sid/record/:year/:month/:day', async(req, res) => {
  const {logger} = req.app.locals;
  const {call_sid, year, month, day} = req.params;

  const account_sid = parseAccountSid(req.originalUrl);
  const r = Account.retrieve(account_sid);
  ...
  switch (o.vendor) {
    case 'aws_s3':
      const getS3Options  = {
        ...o,
        key: `${year}/${month}/${day}/${call_sid}.raw`
      };
      const stream = getS3Object(logger, getS3Options);
      stream.pipe(res);
      break;
    default:
      logger.error(`There is no handler for fetching record from ${o.vendor}`);
