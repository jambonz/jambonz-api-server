...
router.get('/:sid/Lcrs', async(req, res) => {
  const logger = req.app.locals.logger;
  try {
    await validateRetrieve(req);
    const service_provider_sid = parseServiceProviderSid(req);
    const lcrs = await Lcr.retrieveAllByServiceProviderSid(service_provider_sid);

    if (req.user.hasScope('account')) {
      return res.status(200).json(lcrs.filter((c) => c.account_sid === req.user.account_sid || !c.account_sid));
    }

    res.status(200).json(lcrs);
  } catch (err) {
    sysError(logger, res, err);
  }
});
router.post('/:sid/Lcrs', async(req, res) => {
  const logger = req.app.locals.logger;
  try {
    validateUpdate(req);
    const service_provider_sid = parseServiceProviderSid(req);
    const uuid = await Lcr.make({...req.body, service_provider_sid});
    res.status(201).json({sid: uuid});
  } catch (err) {
    sysError(logger, res, err);
  }
});
router.put('/:sid/Lcrs/:lcr_sid', async(req, res) => {
  const logger = req.app.locals.logger;
  try {
    validateUpdate(req);
    const sid = parseLcrSid(req);
    const rowsAffected = await Lcr.update(sid, req.body);
    if (rowsAffected === 0) {
      return res.sendStatus(404);
    }
    res.status(204).end();
