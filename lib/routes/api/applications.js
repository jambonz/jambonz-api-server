...
router.post('/', async(req, res) => {
  ...
  try {
    ...
    if (obj['env_vars']) {
      const appenvschema = fetchAppEnvSchema(obj['call_hook']);
      const errors = validateAppEnvData(appenvschema, obj['env_vars']);
      if (errors) {
        sysError(logger, res, errors);
      } else {
        obj['env_vars'] = encrypt(obj['env_vars']);
      }
    }
...
router.get('/:sid', async(req, res) => {
  ...
  try {
    ...
    if (results[0].env_vars) {
      results[0].env_vars = decrypt(results[0].env_vars);
    }
