...
const hasAccountPermissions = (req, res, next) => {
  ...
  if (req.user.hasScope('account')) {
    ...
    if (account_sid === req.user.account_sid && service_provider_sid === req.user.service_provider_sid) return next();
  }
  res.status(403).json({
    status: 'fail',
...
const hasServiceProviderPermissions = (req, res, next) => {
  ...
  if (req.user.hasScope('account')) {
    ...
    if (service_provider_sid === req.user.service_provider_sid && account_sid === req.user.account_sid) return next();
  }
  res.status(403).json({
    status: 'fail',
    message: 'insufficient privileges'
