...
const getSubspaceJWT = async(id, secret) => {
  ...
  // subspace JWTs created with the api cant make sip teleports for some reason...
  // so you want to copy yours from the subspace portal
  // create a sip teleport and copy the JWT from the web inspector network tab
  ...
  const getJwt = bent('POST', 200, 'https://id.subspace.com');
  const jwtRes = await getJwt('/oauth/token', body);
...
const enableSubspace = async(opts) => {
  ...
  const getTeleport = bent('POST', 200, 'https://api.subspace.com');
  const teleportRes = await getTeleport('/v1/sip-teleports',
    {
      destination: `sip:${opts.sip_realm}`,
      name: 'Jambonz',
      status: 'ENABLED'
    },
    {
  ...
  const teleport = await teleportRes.json();
  return teleport;
};

const disableSubspace = async(opts) => {
  const accessToken = await getSubspaceJWT(opts.subspace_client_id, opts.subspace_client_secret);
  const deleteTeleport = bent('DELETE', 'https://api.subspace.com');

  const teleport = await deleteTeleport(`/v1/sip-teleports/${opts.subspace_sip_teleport_id}`, {}, {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${accessToken}`
  });
