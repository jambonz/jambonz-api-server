...
const commonCheck = async(req) => {
  ...
  if (req.body.password) {
    ...
    req.body.hashed_password = passwordHash;
...
const validateAdd = async(req) => {
  ...
  const clients = await Client.retrieveByAccountSidAndUserName(req.body.account_sid, req.body.user_name);
  if (clients.length) {
    throw new DbErrorBadRequest('the client\'s user_name already exists');
  }
};
