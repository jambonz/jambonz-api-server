...
const { getSpeechCredential } = require('../../utils/speech-utils');
...
router.post('/Synthesize', async(req, res) => {
  ...
  try {
    ...
    const speechCredential = credentials[0];
    const cred = getSpeechCredential(speechCredential);
    ...
    let model;
    if (speechCredential.vendor === 'nuance' && voice) {
    ...
    const { filePath } = await synthAudio(stats, {
      ...
      text,
      vendor: speechCredential.vendor,
      language,
      voice,
      engine,
      model,
