      ...
      if (!isBinary) {
        ...
        if (account && account.length && account[0].bucket_credential) {
          ...
          if (obj.tags && obj.tags.length) {
            socket.upload.on('uploaded', async function(details) {
              logger.info({details}, `audio ${callSid} successfully uploadded, start tagging the object`);
              ...
              await s3.putObjectTagging(putTaggingParams).promise();
              logger.info({details}, `audio ${callSid} tagged`);
            });
          }

          /* start streaming data */
          const wavWriter = new wav.Writer({ channels: 2, sampleRate, bitDepth: 16 });
          const duplex = Websocket.createWebSocketStream(socket);
          duplex.pipe(wavWriter).pipe(socket.upload);
        } else {
