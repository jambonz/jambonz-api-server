...
class RawToWavConverter extends Transform {
  ...
  decodeRawPCM(rawPCM, sampleRate) {
    const audioContext = new AudioContext();
    const numChannels = 1;
    const buffer = audioContext.createBuffer(numChannels, rawPCM.length, sampleRate);
    return buffer;
  }

  _transform(chunk, encoding, callback) {
    const modifiedData = toWav(this.decodeRawPCM(new Float32Array(chunk), this.sampleRate));
    this.push(new Float32Array(modifiedData));
    callback();
  }
}
    ...
    try {
      if (!isBinary) {
        ...
        if (account && account.length && account[0].bucket_credential) {
          ...
          const converter = new RawToWavConverter(sampleRate);
