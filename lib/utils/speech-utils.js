...
const SttAmiVoiceLanguagesVoices = require('./speech-data/stt-amivoice');
const SttAmiVoiceModels = require('./speech-data/stt-amivoice-models');
...
const testAmiVoiceStt = async(logger, credentials) => {
  const {api_key} = credentials;
  
  logger.debug({credentials}, 'testAmiVoiceStt');
  
  // AMI Voice API key validation - should be a long hex string
  if (!api_key || api_key.length < 40) {
    throw new Error('Invalid AMI Voice API key format - key should be at least 40 characters long');
  }

  try {
    const formData = new FormData();
    formData.append('u', api_key);
    formData.append('d', '-a-general-en');
    
    // Read the audio file and create a proper file blob
    const audioBuffer = fs.readFileSync(`${__dirname}/../../data/test_audio.wav`);
    const audioBlob = new Blob([audioBuffer], { type: 'audio/wav' });
    formData.append('a', audioBlob, 'test_audio.wav');

    const response = await fetch('https://acp-api.amivoice.com/v1/nolog/recognize', {
      method: 'POST',
      headers: {
        'User-Agent': 'jambonz-amivoice-client/1.0'
      },
      body: formData
    });

    const result = await response.text();
    logger.debug({result, status: response.status}, 'AMI Voice API response');
    
    if (!response.ok) {
      logger.error({response: result, status: response.status}, 'AMI Voice API error');
      
      // Try to parse the error response
      try {
        const errorData = JSON.parse(result);
        if (errorData.message) {
          throw new Error(`AMI Voice API error: ${errorData.message}`);
        }
      } catch (parseError) {
        // If we can't parse the error, use the original response
      }
      
      throw new Error(`AMI Voice API returned status ${response.status}: ${result}`);
    }
    ...
    try {
      ...
      logger.error({result, parseError}, 'Failed to parse AMI Voice response');
      throw new Error('Invalid JSON response from AMI Voice API');
    }

    // Check if the response indicates success
    if (parsedResult.message && parsedResult.message.includes('illegal service authorization')) {
      throw new Error('AMI Voice API key is invalid or unauthorized');
    }

    if (parsedResult.code && parsedResult.code !== '0' && parsedResult.code !== 0) {
      throw new Error(`AMI Voice API error code: ${parsedResult.code}, message: ${parsedResult.message || 'Unknown error'}`);
    }

    // Check for successful transcription result
    if (parsedResult.results && Array.isArray(parsedResult.results) && parsedResult.results.length > 0) {
      logger.debug('testAmiVoiceStt success - transcription received');
      return parsedResult;
    } else if (parsedResult.text) {
      logger.debug('testAmiVoiceStt success - transcription received');
      return parsedResult;
    } else {
      logger.debug('testAmiVoiceStt success - API key validated');
      return parsedResult;
    }
    
  } catch (error) {
    logger.error({error}, 'AMI Voice STT test failed');
    throw error;
  }
};
...
async function getLanguagesAndVoicesForVendor(logger, vendor, credential, getTtsVoices) {
  switch (vendor) {
    ...
    case 'amivoice':
      return await getLanguagesVoicesForAmiVoice(credential, getTtsVoices, logger);
    case 'voxist':
...
async function getLanguagesVoicesForAmiVoice(credential) {
  return tranform(undefined, SttAmiVoiceLanguagesVoices, undefined, SttAmiVoiceModels);
}
