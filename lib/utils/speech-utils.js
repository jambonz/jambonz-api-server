...
const testAmiVoiceStt = async(logger, credentials) => {
  ...
  // AMI Voice uses WebSocket connections, so we'll validate the API key format
  // and check if it's properly structured (40+ characters long)
  if (!api_key || api_key.length < 40) {
    throw new Error('Invalid AMI Voice API key format');
  }
  const formData = new FormData();
  ...
  if (!response.ok) {
    logger.error({response}, 'Error retrieving AMI Voice transcript');
    throw new Error('failed to get transcript');
  }

  const result = await response.text();
  logger.debug({result}, 'got transcription from AMI Voice');
  
  if (!result || result.trim() === '') {
    throw new Error('No transcript returned from AMI Voice');
  }

  return result;
  // For a more comprehensive test, you could attempt to establish a WebSocket connection
  // to the AMI Voice endpoint, but for now we'll just validate the key format
  logger.debug('testAmiVoiceStt success - API key validated');
  return {status: 'ok', message: 'AMI Voice API key validated successfully'};
};
