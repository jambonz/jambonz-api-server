...
/**
 * speechmatics_stt_uri is stored as a bare hostname - eu2.rt.speechmatics.com and friends for
 * the hosted service, or the hostname of an on-prem container - while the sdk wants a full url
 * that it appends the language to. At call time the stored value is handed to
 * mod_speechmatics_transcribe as SPEECHMATICS_HOST, which always dials wss on port 443 with a
 * path of /v2, so test exactly that. Accepting a shape the runtime cannot dial (a full url, a
 * port) would report a credential as good that then fails on every call, and quietly falling
 * back to a default host would do the same for a credential that has lost the field - the
 * feature server omits SPEECHMATICS_HOST entirely when it is unset, and the module then
 * refuses to start the session.
 */
const speechmaticsRealtimeUrl = (speechmatics_stt_uri) => {
  const host = (speechmatics_stt_uri || '').trim();
  if (!host) throw new Error('speechmatics_stt_uri is not set on this speech credential');
  if (!/^[a-z0-9.-]+$/i.test(host)) {
    throw new Error(
      `invalid speechmatics_stt_uri '${host}': expected a bare hostname, e.g. eu2.rt.speechmatics.com`);
  }
  return `wss://${host}/v2`;
};
...
const testSpeechmaticsStt = async(logger, credentials) => {
  ...
  const realtimeUrl = speechmaticsRealtimeUrl(speechmatics_stt_uri);
  return new Promise(async(resolve, reject) => {
    try {
      const session = new RealtimeSession({ apiKey: api_key, realtimeUrl });
      let transcription = '';
