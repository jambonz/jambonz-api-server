...
const testWhisper = async(logger, synthAudio, credentials) => {
  try {
    ...
    logger.info({err}, 'test whisper returned error');
    throw err;
  }
};

const testPlayHT = async(logger, credentials) => {
  const {api_key, user_id} = credentials;
  try {
    const getJSON = bent('json', {
    ...
    await getJSON('https://api.play.ht/api/v2/voices');
  } catch (err) {
    logger.info({err}, 'test playHT returned error');
    throw err;
  }
};
