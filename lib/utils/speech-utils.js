...
function decryptLlmCredential(obj, credential, logger, isObscureKey = true) {
  switch (obj.vendor) {
    case 'openai':
      ...
      break;
    case 'anthropic':
      const a = JSON.parse(decrypt(credential));
      obj.api_key = isObscureKey ? obscureKey(a.api_key) : a.api_key;
      break;
    default:
      logger.error(`decryptLlmCredential: unsupported vendor ${obj.vendor}`);
      throw new Error(`unsupported vendor ${obj.vendor}`);
  }
}
